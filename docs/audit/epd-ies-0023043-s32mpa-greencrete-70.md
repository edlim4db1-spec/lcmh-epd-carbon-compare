# Audit — EPD-IES-23043

- **Source PDF:** `epd-ies-0023043-s32mpa-greencrete-70.pdf` (16 pp)
- **Programme:** The international EPD® system, www.environdec.com
- **PCR / standard:** 2019:14 Construction Products, Version 1.3.4, 2024-04- · None · EF 3.1
- **Published / valid until:** None / 2025-06-20
- **Extraction confidence:** high
- **Verified by / checked by:** main-agent source-read fix / None

**Number traceability (cross_validate):** 30 declared cells checked, 0 problem(s). **Schema:** 0 error(s).

## System boundary (per module)

| A1 | A2 | A3 | A1-A3 | A4 | A5 | B1 | B2 | B3 | B4 | B5 | B6 | B7 | C1 | C2 | C3 | C4 | D |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ND | ND | ND | ✓ | ND | ND | ND | ND | ND | ND | ND | ND | ND | ✓ | ✓ | ✓ | ✓ | ✓ |

## Product 1: Premix Concrete – S32MPa GreenCrete 70

- **Manufacturer:** Piave Premix Concrete Pty Ltd
- **Compressive strength:** 32 MPa (class None, status found) — p.1
- **Location:** Port Melbourne (AU)
- **Declared unit:** 1 m3 · mass 2373.0 kg

### GWP-total (kg CO2 eq) — provenance per module

| Module | Value | Raw (verbatim) | Status | Page | Section |
|---|---|---|---|---|---|
| A1-A3 | 134.0 | `1.34E+02` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
| C1 | 5.67 | `5.67E+00` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
| C2 | 21.6 | `2.16E+01` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
| C3 | 12.5 | `1.25E+01` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
| C4 | 2.29 | `2.29E+00` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
| D | -1.61 | `‐1.61E+00` | declared | 11 | CORE ENVIRONMENTAL IMPACT INDICATORS – EN 1580 |
